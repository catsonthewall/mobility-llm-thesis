"""
Mobility feature classification — gender
=============================================================================
Target  : gender  (Female / Male; other values dropped)
...
Input   : feature_matrix_normalised.csv  — already MinMax [0,1], no rescaling
=============================================================================
"""
...
out_dir     = Path('/data/baliu/thesis/03_method/00_ml_classification/gender')
...
# gender values to keep — everything else is dropped
...
def load_and_merge():
    print("=" * 70)
    print("loading data  |  target: gender")
    print("=" * 70)
    ...
    le  = LabelEncoder().fit(merged['gender'].values)
    y   = le.transform(merged['gender'].values)

    print(f"  classes : {list(le.classes_)}  "
          f"(encoded: {list(range(len(le.classes_)))})")

    # class balance check
    unique, counts = np.unique(y, return_counts=True)
    for cls, n in zip(le.classes_, counts):
        print(f"    {cls:<10} {n:>5}  ({n/len(y)*100:.1f}%)")
...
def train_xgboost(X_train, y_train, X_test, y_test, feature_cols):
    ...
    if n_classes > 2:
        final_kwargs['num_class'] = n_classes

    final = xgb.XGBClassifier(**final_kwargs)
    final.fit(X_train, y_train, sample_weight=_weights(y_train), verbose=False)
...
def _confusion_plot(y_true, y_pred, labels, title, path):
    ...
    plt.figure(figsize=(max(6, len(labels) + 2), max(5, len(labels) + 1)))
...
def print_comparison(models_dict, X_test, y_test, le):
    ...
    for name, info in models_dict.items():
        ...
        rows.append({
            ...
            'f1_female'  : round(f1_score(y_test, y_pred,
                                          labels=[list(le.classes_).index('Female')],
                                          average='macro'), 4),
            'f1_male'    : round(f1_score(y_test, y_pred,
                                          labels=[list(le.classes_).index('Male')],
                                          average='macro'), 4),
        })
...
def plot_comparison_bar(comp_df):
    x, w = np.arange(len(comp_df)), 0.20
    metrics = ['accuracy', 'f1_macro', 'f1_female', 'f1_male']
    colors  = ['#2196F3', '#4CAF50', '#E91E63', '#FF9800']
    ...
    ax.set_ylabel('score')
    ax.set_ylim(0, 1.08)
    ax.set_title('model comparison — gender prediction',
...
def plot_feature_importance(rf_imp, xgb_imp, n_top=20):
    ...
    for ax, imp, title in zip(axes,
        ...
        ax.set_title(f'{title} — feature importance  (target: gender)',
...
def main():
    print("=" * 70)
    print("mobility feature classification — gender")
